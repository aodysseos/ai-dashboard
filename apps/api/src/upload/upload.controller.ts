...
export class UploadController {
   ...
   */
  generatePresignedUrls = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const uploadRequests = files.map(file => ({
        key: S3Service.generateS3Key(file.filename),
        contentType: file.contentType
      }));

      const uploads = await this.s3Service.generatePresignedUploadUrls(uploadRequests);
